...
"<div class='card'>"
"<h2>Profiles</h2>"
"<div id='profilesList' class='label'>(loading...)</div>"
"<hr/>"
...
"    if(!list || !Array.isArray(list)) { document.getElementById('profilesList').textContent='(no profiles)'; return; }"
"    const container = document.getElementById('profilesList');"
"    container.innerHTML = '';"
"    list.forEach(name=>{"
"      const el = document.createElement('div'); el.className='label'; el.style.margin='6px 0';"
"      el.textContent = name;"
"      el.onclick = ()=>{ loadProfileToEditor(name); };"
"      container.appendChild(el);"
"    });"
"  }).catch(()=>{ document.getElementById('profilesList').textContent='(failed)'; });"
"}"
