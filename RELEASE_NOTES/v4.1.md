...
- Responsive behavior:
  - Sensor line wraps at **≤ 560px** to avoid layout breaks.
  - Skins FAB collapses into a compact stacked layout at **≤ 950px**.
  - Device/Source selects are responsive and synchronize widths with special handling in the **480–560px** range.
...
- **Deprecated**: the CLI flag `--thermal-zone` and the `set-thermal-zone` control command have been removed; use the `thermal_zone` config key (set to -1 for auto) or the new `--sensor-source` / `--sensor` options instead.
