...
- Priority order: explicit `--sensor` path → `--thermal-zone` → auto-detect  (Note: in v4.1 this was expanded — the daemon prefers HWMon when available and adds the `--sensor-source <auto|hwmon|thermal>` setting to control source preference)
