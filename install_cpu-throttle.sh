...
regenerate_asset_headers() {
    if [[ -f include/assets_generated.h ]]; then
        return 0
    fi
    if [[ ! -d assets ]]; then
        return 1
    fi
    echo "➡️ Regenerating include headers from assets/ (fallback)"
    mkdir -p include
    if command -v xxd >/dev/null 2>&1; then
        xxd -i assets/index.html > include/index_html.h 2>/dev/null || true
        xxd -i assets/main.js > include/main_js.h 2>/dev/null || true
        xxd -i assets/styles.css > include/styles_css.h 2>/dev/null || true
        if [[ -f assets/favicon.ico ]]; then
            xxd -i assets/favicon.ico > include/favicon_ico.h 2>/dev/null || true
        else
    cat > include/favicon_ico.h <<'EOF'
    /* autogenerated empty favicon header (no assets/favicon.ico provided) */
    unsigned char favicon_ico_data[] = { 0 };
    unsigned int favicon_ico_data_len = 0;
EOF
        fi
    cat > include/assets_generated.h <<'EOF'
    /* generated by make assets */
    #define USE_ASSET_HEADERS 1
EOF
        return 0
    else
        echo "➡️ xxd not available; cannot regenerate asset headers automatically" >&2
        return 1
    fi
}
ENABLE_TUI=0
...
if [[ "$SKIP_BUILD" != true ]]; then
    ...
    if [[ ${BUILD_FALLBACK:-false} == true ]]; then
        ...
        # After possibly changing directory into a nested source tree, attempt
        # to regenerate asset headers relative to the current working directory
        # so that includes like "include/assets_generated.h" resolve correctly.
        echo "➡️ Build working dir: $(pwd)"
        echo "➡️ Listing assets/ and include/ before compile"
        ls -la assets 2>/dev/null || true
        ls -la include 2>/dev/null || true
        if [[ -f include/assets_generated.h ]]; then
            echo "➡️ include/assets_generated.h exists; preview:" && sed -n '1,20p' include/assets_generated.h || true
        else
            if ! regenerate_asset_headers; then
                echo "⚠️ Could not regenerate asset headers automatically; compilation may fail if headers are required."
            fi
        fi
        ...
        if [[ -f "$SOURCE_FILE" ]]; then
            ...
            # Use the same compiler flags as the Makefile for deterministic builds
            CC=${CC:-gcc}
            CFLAGS='-std=gnu11 -O2 -Wall -Wextra -pthread'
            echo "➡️ Compiling with: $CC $CFLAGS -o $BINARY_NAME $SOURCE_FILE $TUI_LDFLAGS"
            "$CC" $CFLAGS -o "$BINARY_NAME" "$SOURCE_FILE" $TUI_LDFLAGS || { echo "❌ Failed to compile $SOURCE_FILE"; exit 1; }
        else
    ...
    # Copy built binary to /tmp for easy comparison between runs
    if [[ -n "${SRC_BIN_PATH:-}" && -f "$SRC_BIN_PATH" ]]; then
        BUILD_LABEL="built"
        if [[ "${FORCE_BUILD:-0}" -eq 1 ]]; then
            BUILD_LABEL="forced"
        elif [[ "${ACTION:-}" == "build" ]]; then
            BUILD_LABEL="interactive"
        fi
        TMP_DST="/tmp/${BINARY_NAME}.${BUILD_LABEL}"
        echo "➡️ Copying built binary to $TMP_DST"
        cp -f "$SRC_BIN_PATH" "$TMP_DST" || true
